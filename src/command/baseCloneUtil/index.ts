...
export default (context: vscode.ExtensionContext) => {
	return vscode.commands.registerCommand(
		"code-templates.base.CloneCommonUtils",
		async (res) => {
			// console.log("CloneTemplate", res);
			const path = res.fsPath || res.filePath;
			const { localREPOPath, localTemplate3Path } = getTemplatePath(context);
			...
			if (fs.existsSync(localREPOPath)) {
				if (fs.existsSync(localTemplate3Path)) {
					const list = getTemplateList(localTemplate3Path);

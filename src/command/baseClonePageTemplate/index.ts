...
export default (context: vscode.ExtensionContext) => {
	return vscode.commands.registerCommand(
		...
		async (res) => {
			...
			const { localREPOPath, templateCategorys } = getTemplatePath(context);
			...
			if (fs.existsSync(localREPOPath)) {
				vscode.window
					.showQuickPick(templateCategorys, {
						placeHolder: "请选择类别",
					})
					.then((item: any) => {
						if (item.path && fs.existsSync(item.path)) {
							const templatePath = item.path;
							const list = getTemplateList(templatePath);
							...
							vscode.window
								...
								.then((valueQuickPick: any) => {
									// 只有选中合法才执行后续逻辑
									if (valueQuickPick) {
										const { fileName, filePath, configInfo } =
											valueQuickPick || {};
										const optionsInput = {

 ...
 */
const copyFiles = (sourcePath, destinationPath, operation) => {
    // 判断源路径是否为文件
    if (fs.statSync(sourcePath).isFile()) {
        ...
        if (!fs.existsSync(destinationPath)) {
            (0, createDirectory_1.default)(destinationPath);
        }
        fs.writeFileSync(path.join(destinationPath, path.basename(sourcePath)), destinationFile);
    }
    else {
        ...
        files.forEach((file) => {
            ...
            if (fs.statSync(sourceFilePath).isDirectory()) {
                ...
                if (!fs.existsSync(destinationFilePath)) {
                    (0, createDirectory_1.default)(destinationFilePath);
                }
